...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  const angleRad = Math.abs(Math.atan2(y2, x2) - Math.atan2(y1, x1));
  return angleRad;
}
...
function getSumOfDigits(num) {
  ...
  const res = strArr.reduce((sum, current) => +sum + +current);
  return res;
}
...
function getRandomInteger(min, max) {
  return Number.parseInt(Math.random(), 10) * (max - min) + min;
}
...
function getCountOfOddNumbers(number) {
  let current = Math.round(Math.abs(number));
  let oddCounter = 0;
  for (; current !== 0; current -= 1) {
    if (current % 2 !== 0) oddCounter += 1;
  }
  return oddCounter;
}
